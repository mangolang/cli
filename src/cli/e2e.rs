...
fn compile_exe() {
    MANGO_EXE.write().unwrap().get_or_insert_with(|| {
        ...
        let build_infos = from_utf8(&result.stdout).expect("could not parse json output of cargo build");
        ...
        assert!(!exes.is_empty(), "could not find executable in cargo output");
...
fn run_cli(args: &[&str]) -> (i32, String, String) {
    MANGO_EXE
        ...
        .unwrap()
        .as_ref()
        .map(|exe_pth| run_cli_with(&exe_pth, args))
        .unwrap()
}

fn run_cli_with(exe_path: &Path, args: &[&str]) -> (i32, String, String) {
    let output = Command::new(exe_path).args(args).output().unwrap();
    (
